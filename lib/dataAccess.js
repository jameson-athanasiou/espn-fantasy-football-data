...
const { standingsMap, statsMap } = require('./propertyMaps');
...
const getSeasonStandings = () => {
    ...
    return new Promise((resolve, reject) => {
        JSDOM.fromURL(url).then((dom) => {
            ...
            const fixedObject = sortedTable.map(individualTable => fixPropertyNames(individualTable, standingsMap));
...
const getSeasonTeamStats = () => {
    ...
    return new Promise((resolve, reject) => {
        JSDOM.fromURL(url).then((dom) => {
            ...
            const fixedObject = sortedTable.map(table => fixPropertyNames(table, statsMap));
